...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         ...
         isTypePredTExp,
         TypePredTExp} from "./TExp";
import { isEmpty, allT, first, rest, NonEmptyList, List, isNonEmptyList } from '../shared/list';
import { Result, makeFailure, bind, makeOk, zipWithResult, either, isFailure, isOk } from '../shared/result';
...
const primTypePredicates: string[] = ['number?', 'boolean?', 'string?', 'any?', 'never?', 'list?', 'pair?', 'symbol?'];
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<TypePredTExp> => {
    ...
    const t = typeofExp(e.rator, tenv);

    if (isOk(t) && isTypePredTExp(t.value))
    {
        return makeOk(t.value);
    }

    return makeFailure("bad bad");
}

export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => //TODO
    either(
        bind (isTypePredApp(ifExp.test, tenv), (tp: TypePredTExp)=> {
            const thenTE = typeofExp(ifExp.then, tenv);
            const altTE = typeofExp(ifExp.alt, tenv);

            return bind(thenTE, (thTE: TExp) => // can calculate the then
                bind(checkCompatibleType(tp.te2, thTE, ifExp), (r: true) => // then and test are comptible
                    bind(altTE, (alTE: TExp) => // can calculate the alt
                        makeOk(makeUnion(tp.te2, alTE)))))  
        }
        ),
        makeOk,
