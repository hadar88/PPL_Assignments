...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, makeBoolTExp, makeTypePredTExp } from './TExp';
...
import { Result, bind, makeFailure, mapResult, makeOk, mapv, isOk, isFailure } from "../shared/result";
...
// ( lambda ( <var-decl>* ) [: is? <TExp>]? <cexp>+ )
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {// To complete
    if (isArray(vars)) {
        const args = mapResult(parseVarDecl, vars);
        const body = mapResult(parseL5CExp, rest[0] === ":" ? rest[1] === "is?" ? rest.slice(3) : rest.slice(2) : rest);

        const returnTE: Result<TExp> = rest[0] === ":" ?  
            rest[1] === "is?" ? 
                bind(args, (args: VarDecl[]) => args.length !== 1 ? 
                    makeFailure("invalid num of args") : 
                        bind(parseTExp(rest[2]), (x: TExp) => makeOk(makeTypePredTExp(args[0].texp, x))))
            : parseTExp(rest[1]) 
