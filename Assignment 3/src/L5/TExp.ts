...
;; <compound-te>  ::= <proc-te> | <tuple-te> | <union-te> | <inter-te> | <diff-te> | <typepredTexp>
...
;; <typePredTExp> ::= (<TExp> -> is? <TExp>) // type-predicates
...
export const isTExp = (x: any): x is TExp => isAtomicTExp(x) || isCompoundTExp(x) || isTVar(x) || isTypePredTExp(x);
...
export const makeDiffTExp = (te1: TExp, te2: TExp): TExp =>
    {
        ...
        else if (isNeverTExp(te2)){
            return te1;
        }
        
        return makeUnionTExp(differenceWith(isSubType, flattenSortUnion([te1]), flattenSortUnion([te2])));
    }

export type TypePredTExp = { tag: "TypePredTExp"; te1: TExp; te2: TExp;};
export const isTypePredTExp = (x: any): x is TypePredTExp => x.tag === "TypePredTExp";
export const makeTypePredTExp = (te1: TExp, te2: TExp): TExp => ({tag: "TypePredTExp", te1: te1, te2: te2});
...
// SubType comparator
export const isSubType = (te1: TExp, te2: TExp): boolean => // (any number) , (union string number boolean)
    (isAnyTExp(te2)) ? true :
    ...
    isAtomicTExp(te1) && isAtomicTExp(te2) ? equals(te1, te2) :
    false;
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> =>
    ...
    (texps.length === 4 && texps[2] === "is?") ? parseTypePredTExp(texps) :
    parseProcTExp(texps);
...
const parseDiffTExp = (texps: Sexp[]): Result<TExp> =>
    mapv(mapResult(parseTExp, texps.slice(1)),
        (tes: TExp[]) => makeDiffTExp(tes[0], tes[1]));

const parseTypePredTExp = (texps: Sexp[]): Result<TExp> =>
    bind(parseTExp(texps[0]), (t1: TExp) => bind(parseTExp(texps[3]), (t2: TExp) => makeOk(makeTypePredTExp(t1, t2))));
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTypePredTExp(x) ? bind(unparseTExp(x.te1), (te1: string) =>
                            bind(unparseTExp(x.te2), (te2: string) =>
                                makeOk(`(${te1} -> is? ${te2})`))) :
