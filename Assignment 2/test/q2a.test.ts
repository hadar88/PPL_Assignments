     ...
     it('test parse/unparse class', () => {
          ...
          (class (a b c) 
            ((first (lambda () a)) 
             (second (lambda () b)) 
             (third (lambda () c))
             (mult-by (lambda (k) (* k a b c)))
             (sum (lambda () (+ a b)))))`),parseL3Exp), x=>makeOk(unparseL3(x)))).to.deep.equal(makeOk(`(class (a b c) ((first (lambda () a)) (second (lambda () b)) (third (lambda () c)) (mult-by (lambda (k) (* k a b c))) (sum (lambda () (+ a b)))))`));
         });
