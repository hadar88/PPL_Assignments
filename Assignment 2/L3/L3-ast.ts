// ===========================================================
// AST type models
import { all, is, map, reduce, zipWith } from "ramda";
import { makeEmptySExp, makeSymbolSExp, SExpValue, makeCompoundSExp, valueToString, isSExp } from './L3-value'
import { first, second, rest, allT, isEmpty, isNonEmptyList, List, cons } from "../shared/list";
...
const isGoodFields = (fields: Sexp): fields is string[] =>
    {
        ...
        return fields.reduce((acc, curr) => acc && (typeof(curr) == "string"), true)
    }
...
export const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> =>
    {
        ...
        const vars = map(f => makeVarDecl(f), fields);

        const actualMethods = methods[0];

        if (!isNonEmptyList<Sexp>(actualMethods)){
            return makeFailure('Malformed methods in "class" expression');
        }

        const functions = map((m) => m[0], actualMethods);
        const values = map((m) => m[1], actualMethods);

        if (!isGoodFields(functions)) {
            return makeFailure('Malformed fields in "class" expression');
        }

        const functionsDecls = map(makeVarDecl, functions);

        const methodsResult = mapResult(parseL3CExp, values);

        if (isFailure(methodsResult))
            return makeFailure('Malformed methods in "class" expression');

        const bindings = zipWith(makeBinding, functions, methodsResult.value);

        return makeOk(makeClassExp(vars, bindings));
    }
...
const unparseClassExp = (cla: ClassExp) : string => 
    `(class (${map((field: VarDecl) => field.var, cla.fields).join(" ")}) (${map((method: Binding) => `(${method.var.var} ${unparseL3(method.val)})`, cla.methods).join(" ")}))`
